...
"<style>"
"body{background:#111;color:#ddd;font-family:ui-monospace,Menlo,monospace;margin:0;height:100vh;height:100dvh;display:flex;flex-direction:column;touch-action:manipulation}"
...
".sys{color:#888}"
"#inp{width:100%;box-sizing:border-box;padding:9px;background:#1a1a1a;border:1px solid #333;color:#ddd;border-radius:4px;font-size:16px;outline:none}"
...
"var colorOf={};"
"function nickColor(n){var h=colorOf[n];"
"if(h===undefined){var used=[],k,i,h2,d,best=0,bd=-1;"
"for(k in colorOf)used.push(colorOf[k]);"
"if(!used.length)h=Math.floor(Math.random()*360);"
"else{for(h2=0;h2<360;h2++){d=360;"
"for(i=0;i<used.length;i++){var dd=Math.abs(used[i]-h2);if(dd>180)dd=360-dd;if(dd<d)d=dd;}"
"if(d>bd){bd=d;best=h2;}}h=best;}"
"colorOf[n]=h;}"
"return 'hsl('+h+',70%,65%)';}"
